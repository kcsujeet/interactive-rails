...
export interface DatabaseConfig extends BaseNodeConfig {
  tables: Array<{
    ...
    columns: Array<{
      name: string;
      type: 'string' | 'text' | 'integer' | 'float' | 'boolean' | 'datetime' | 'references';
    }>;
    indexes: Array<{
      columns: string[];
      unique?: boolean;
...
export const NODE_DEFAULTS: Record<NodeType, Partial<BaseNode>> = {
  ...
  cache: {
    ...
    config: { strategy: 'cache_aside', store: 'redis', ttl: 3600 } as CacheConfig,
  },
  view: {
    ports: [
  ...
  response: {
    ports: [
      { id: 'in', type: 'input', dataType: 'any', label: 'Response' },
    ],
