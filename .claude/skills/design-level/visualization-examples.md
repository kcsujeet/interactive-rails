...
- [ ] **Nodes at the same level are the same kind of thing.** All nodes in a diagram should be peers of the same category: all pipeline stages (Controller, Model, Serializer), all systems (App, Database, Stripe), or all actors (Customer, Admin). If a node is a different category from its neighbors (e.g., a "Timeout" node among system nodes like "App Server" and "Stripe"), the player will assume it's a separate system. Show it as a sub-element inside its parent node instead. Case study: L38's original design had Timeout, Retry, and Circuit Breaker as peer nodes alongside App Server and Stripe, implying they were separate services. The fix: show the middleware as sub-elements inside the App Server node.
