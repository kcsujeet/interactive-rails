...
pipelineRoutes.get('/dungeons', authMiddleware, async (c) => {
	...
	const [progressRow, completionsRows] = await Promise.all([
		...
		db
			.prepare(
				'SELECT dungeon_id, stars_earned, best_stability FROM dungeon_completions WHERE user_id = ?',
			)
			.bind(userId)
			.all(),
	]);
	...
	const completions = new Map(
		(completionsRows.results || []).map((r) => [
			r.dungeon_id,
			{ stars: r.stars_earned, stability: r.best_stability },
		]),
	);
	...
	const dungeons = [
		...
		},
		{
			...
			description:
				'Implement caching strategies to dramatically improve performance',
			difficulty: 3,
			requiredLevel: 1,
		...
		},
		{
			...
			description:
				'A boss dungeon that tests all your database optimization skills',
			difficulty: 5,
			requiredLevel: 1,
			requiredDungeons: [
				'tutorial-n-plus-one',
	...
	const dungeonsWithStatus = dungeons.map((dungeon) => {
		...
		const meetsPrereqs = dungeon.requiredDungeons.every((id) =>
			completedIds.has(id),
		);
...
pipelineRoutes.post(
	...
	async (c) => {
		...
		const existing = await db
			.prepare(
				'SELECT * FROM dungeon_completions WHERE user_id = ? AND dungeon_id = ?',
			)
		...
		if (existing) {
			...
			const isNewBestStability =
				finalStability > (existing.best_stability as number);
			const isNewBestTime =
				timeToComplete < (existing.best_time_seconds as number);
            ...
            WHERE user_id = ? AND dungeon_id = ?`,
				...
				await db
					.prepare(
						'UPDATE dungeon_completions SET attempts = attempts + 1 WHERE user_id = ? AND dungeon_id = ?',
					)
					.bind(userId, dungeonId)
					.run();
			}
		...
		if (progress) {
			if (stackChoices) {
				await db
					.prepare(
						'UPDATE player_progress SET stack_choices = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?',
					)
			...
			const dungeonsCompleted =
				(progress.dungeons_completed as number) + (existing ? 0 : 1);
			const starsEarned =
				(progress.total_stars_earned as number) +
				(existing
					? Math.max(0, stars - (existing.stars_earned as number))
					: stars);
        ...
        VALUES (?, ?, ?, 1, ?, CURRENT_TIMESTAMP, ?)`,
			...
			.bind(
				userId,
				xpReward,
				levelFromXp(xpReward),
				stars,
				stackChoices ? JSON.stringify(stackChoices) : null,
			)
			.run();
...
pipelineRoutes.get('/progress', authMiddleware, async (c) => {
	...
	const completedLevels = (completionsRows.results || []).map(
		(r) => r.dungeon_id as string,
	);
	...
	const stackChoices = progress?.stack_choices
		? JSON.parse(progress.stack_choices as string)
		: null;
...
pipelineRoutes.post(
	...
	async (c) => {
		const userId = c.get('userId');
		const { completedLevels, levelProgress, stackChoices } =
			c.req.valid('json');
		const db = c.env.DB;
			...
			return c.json(
				{
					...
					error: {
						...
						message:
							'Guest progress has already been imported for this account.',
					},
					meta: {
						requestId: c.get('requestId'),
						timestamp: new Date().toISOString(),
		...
		for (const levelId of completedLevels) {
			...
			if (existing) {
				const isNewBestStability =
					bestScore > (existing.best_stability as number);
				const isNewBestStars = stars > (existing.stars_earned as number);
		...
		const currentCompleted =
			(updatedProgress?.dungeons_completed as number) || 0;
        ...
        WHERE user_id = ?`,
			...
			.bind(
				...
				stackChoices
					? JSON.stringify(stackChoices)
					: updatedProgress?.stack_choices || null,
				userId,
			)
			.run();
