...
function setAuthCookie(c: { env: Env; header: (name: string, value: string) => void }, token: string): void {
  const isProduction = c.env.ENVIRONMENT === 'production';

  setCookie(c as unknown as Parameters<typeof setCookie>[0], AUTH.COOKIE.NAME, token, {
    httpOnly: true,
    secure: isProduction,
    sameSite: 'Lax',
...
function formatUserResponse(user: { id: string; email: string; username: string }) {
  return {
    id: user.id,
    email: user.email,
