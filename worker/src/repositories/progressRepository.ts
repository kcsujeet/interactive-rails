...
import type { DungeonProgress, UserAchievement, UserProgress } from '../types';
...
export class ProgressRepository {
	...
	async getStats(userId: string): Promise<ProgressStats> {
		...
		return {
			...
			accuracy: challengeStats?.total
				? Math.round(
						((challengeStats.correct ?? 0) / challengeStats.total) * 100,
					)
				: 0,
	...
	async getDungeonProgress(
		...
		const result = await this.db
			.prepare(
				'SELECT * FROM dungeon_progress WHERE user_id = ? AND realm_id = ?',
			)
			.bind(userId, realmId)
			.all<DungeonProgress>();
	...
	async getCompletedDungeonsByRealm(
		userId: string,
