...
const level27LoadBalancing: Level = {
	...
	trigger: {
		type: 'scaling',
		description:
			'Single server cannot handle traffic. Time to scale horizontally.',
	},
...
Rack::Attack.throttled_responder = lambda do |req|
  [429, {'Content-Type' => 'application/json'}, [{error: 'Rate limit exceeded'}.to_json]]
		...
		commonMistakes: [
			'Rate limiting by user only (not IP)',
			'Too generous limits',
			'No allow list for health checks',
		],
		whenToUse: 'Every production application.',
		furtherReading: [
			{ title: 'Rack::Attack', url: 'https://github.com/rack/rack-attack' },
		],
	},
	hint: {
		delay: 20,
		text: 'Use Rack::Attack with different limits per endpoint.',
	},
};
...
production:
  ...
  url: <%= ENV['PGBOUNCER_URL'] %>`,
		commonMistakes: [
			'Pool too small',
			'Not accounting for Sidekiq',
			'Forgetting worker boot hook',
		],
		whenToUse: 'Every Rails app with multiple processes/threads.',
		furtherReading: [
			{
				title: 'Rails Connection Pooling',
...
export const actFive: Act = {
	...
	description:
		'Master Rails infrastructure: load balancing, CDN, rate limiting, connection pooling, and zero-downtime deployments.',
	levels: [
