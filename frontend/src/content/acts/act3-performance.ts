...
const level16N1Problem: Level = {
  ...
  trigger: {
    ...
    description: 'Page load is 5 seconds. Database shows 101 queries for 100 posts.',
  },
...
# 100 posts = 101 queries!`,
    ...
    furtherReading: [{ title: 'Bullet Gem', url: 'https://github.com/flyerhzm/bullet' }],
  },
...
Post.includes(comments: :user)`,
    commonMistakes: ['Using eager_load when includes works', 'Loading too much data'],
    whenToUse: 'includes is usually right. eager_load when filtering on associations.',
    furtherReading: [{ title: 'ActiveRecord Querying', url: 'https://guides.rubyonrails.org/active_record_querying.html#eager-loading-associations' }],
  },
  hint: { delay: 20, text: 'Use includes for most cases, eager_load when filtering.' },
};
...
add_index :posts, [:published, :created_at]`,
  ...
  hint: { delay: 20, text: 'Use pluck for values, add indexes for slow queries.' },
};
...
@pagy, @posts = pagy(Post.all, items: 25)`,
    commonMistakes: ['Offset pagination on millions of rows', 'Not using database for pagination'],
...
stale?(@post)`,
    ...
    furtherReading: [{ title: 'Rails Caching Guide', url: 'https://guides.rubyonrails.org/caching_with_rails.html' }],
  },
  hint: { delay: 20, text: 'Start with fragment caching, add Redis for low-level.' },
};
...
const level21BackgroundJobs: Level = {
  ...
  trigger: {
    type: 'incident',
    description: 'PDF generation blocks requests for 30 seconds. Users are frustrated.',
  },
  startingPipeline: { nodes: [], connections: [] },
  problem: {
...
:queues:
  ...
  hint: { delay: 20, text: 'Use perform_later for async, pass IDs not objects.' },
};
...
export const actThree: Act = {
  ...
  description: 'Master Rails performance: fix N+1 queries, implement caching, optimize database queries, and use background jobs.',
  levels: [
    level16N1Problem,
    level17EagerLoading,
