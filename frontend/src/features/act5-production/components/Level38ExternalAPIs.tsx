...
export function Level38ExternalAPIs({
	onComplete,
}: LevelComponentProps) {
	...
	const successfulCalls = calls.filter(
		(c) => c.status === 'success' || c.status === 'fallback',
	).length;
	const failedCalls = calls.filter(
		(c) => c.status === 'timeout' || c.status === 'error',
	).length;
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			const responseTime = isAPIUnreliable
	...
	const handleComplete = async () => {
		const success = await completeLevel('act5-level38-external-apis', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
						...
						<div className="grid grid-cols-4 gap-4 mb-6">
							{(
								...
								>
									...
									<div
										className={`font-semibold ${config[item.key as keyof APIConfig] ? 'text-success' : 'text-foreground'}`}
									>
										{item.name}
									</div>
							...
							<div className="p-6">
								<div className="flex items-center justify-between">
									...
									<div className="flex-1 flex items-center justify-center">
										<div
											className={`h-1 flex-1 ${config.timeout ? 'bg-success' : 'bg-border'}`}
										/>
										{config.timeout && (
											<div className="px-2 py-1 bg-success/20 rounded text-xs text-success mx-2 flex items-center gap-1">
												<Timer className="w-3 h-3" /> 3s
											</div>
										)}
										{config.retries && (
											<div className="px-2 py-1 bg-primary/20 rounded text-xs text-primary mx-2 flex items-center gap-1">
												<RefreshCw className="w-3 h-3" /> x2
											</div>
										)}
										{config.circuitBreaker && (
											<div
												className={`px-2 py-1 rounded text-xs mx-2 flex items-center gap-1 ${
													circuitState === 'closed'
											...
											>
												<Unplug className="w-3 h-3" /> {circuitState}
											</div>
										)}
										<div
											className={`h-1 flex-1 ${config.fallback ? 'bg-warning' : 'bg-border'}`}
										/>
									</div>
						...
						<div className="bg-card rounded-xl border border-border overflow-hidden">
							...
							<div className="p-4 h-48 overflow-y-auto font-mono text-xs space-y-1">
								{calls.length === 0 ? (
									...
									calls.map((call) => (
										<div className="flex items-center gap-3" key={call.id}>
											<span
