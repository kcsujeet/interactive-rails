...
export function Level28CDN({ onComplete, onExit }: LevelComponentProps) {
	...
	const simulateRequest = () => {
		...
		const isStaticAsset = ['app.js', 'styles.css', 'fonts.woff2'].includes(
			asset,
		);
		...
		if (config.enabled) {
			...
			const isCached =
				config.staticAssets &&
				(isStaticAsset || (config.imageOptimization && isImage));
	...
	const cacheHitRate =
		requests.length > 0
			? Math.round(
					(requests.filter((r) => r.source === 'edge').length /
						requests.length) *
						100,
				)
			: 0;
			...
			<LeftPanel>
				...
				>
					...
					<div className="p-4 border-t border-border space-y-2">
						<Button className="w-full" onClick={simulateRequest}>
							Simulate Request
						</Button>
						{config.enabled && (
							<Button
							...
							<div className="p-4 grid grid-cols-4 gap-3">
								{[
									...
									},
									{
										...
										name: 'Image Optim',
										icon: '🖼️',
										desc: 'Auto-resize, WebP',
									},
									{
										key: 'cacheControl',
										name: 'Cache Headers',
										icon: '⏱️',
										desc: 'max-age, s-maxage',
									},
								].map((item) => (
									<Button
									...
									>
										...
										<div
											className={`text-xs font-semibold ${config[item.key as keyof CDNConfig] ? 'text-success' : 'text-foreground'}`}
										>
											{item.name}
										</div>
										<div className="text-xs text-muted-foreground">
									...
									{config.enabled && (
										<div className="flex-1 mx-8">
											<div className="flex justify-center gap-4">
												{locations.map((loc) => (
													<div className="text-center" key={loc.id}>
														<div
														...
														<div
															className={`text-xs ${loc.cached ? 'text-success' : 'text-muted-foreground'}`}
														>
									...
									requests.map((req) => (
										...
										>
											<div className="flex items-center gap-3">
												<span
													className={`w-2 h-2 rounded-full ${req.source === 'edge' ? 'bg-success' : 'bg-warning'}`}
												/>
											...
											<div className="flex items-center gap-3">
												...
												<span
													className={`text-sm ${req.latency < 100 ? 'text-success' : 'text-warning'}`}
												>
													{req.latency}ms
												</span>
											</div>
