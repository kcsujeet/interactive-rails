...
export function Level31Deployments({
	onComplete,
	onExit,
}: LevelComponentProps) {
			...
			switch (strategy) {
				case 'blue-green':
					...
					if (step < 8) {
						...
						setServers((prev) =>
							prev.map((s) => ({ ...s, version: 'v2', traffic: 33 })),
						);
					}
					break;
				...
				case 'rolling':
					...
					if (step === 3) {
						...
						setServers((prev) =>
							prev.map((s, i) =>
								i === 0
									? { ...s, version: 'v2', status: 'running', traffic: 33 }
									: i === 1
										? { ...s, status: 'deploying', traffic: 0 }
										: { ...s, traffic: 67 },
							),
	...
	const handleComplete = async () => {
		const success = await completeLevel('act5-level31-deployments', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
						...
						<div className="space-y-2">
							...
							<div className="grid grid-cols-2 gap-2 mt-3">
								<div className="bg-card p-2 rounded text-center">
									<div
										className={`text-lg font-bold ${downtime === 0 ? 'text-success' : 'text-destructive'}`}
									>
										{downtime}s
									</div>
								...
								<div className="bg-card p-2 rounded text-center">
									<div
										className={`text-lg font-bold ${errorRate < 1 ? 'text-success' : 'text-destructive'}`}
									>
										{errorRate}%
									</div>
								...
								{[
									{
										...
										name: 'Blue-Green',
										icon: '🔵🟢',
										desc: 'Instant switch, easy rollback',
										pros: ['Instant rollback', 'No mixed versions'],
										cons: ['Needs 2x resources'],
									},
									{
										id: 'rolling',
										name: 'Rolling',
										icon: '🔄',
										desc: 'Update one by one',
									...
									},
									{
										...
										name: 'Canary',
										icon: '🐤',
										desc: 'Test with small traffic',
										pros: ['Test in production', 'Catch issues early'],
										cons: ['Complex routing', 'Needs monitoring'],
									},
								].map((s) => (
									<Button
										...
										key={s.id}
										onClick={() =>
											!isDeploying && setStrategy(s.id as DeploymentStrategy)
										}
										variant={strategy === s.id ? 'default' : 'outline'}
									>
										<div className="text-2xl mb-2">{s.icon}</div>
										<div
											className={`font-semibold ${strategy === s.id ? 'text-primary' : 'text-foreground'}`}
										>
											{s.name}
										</div>
