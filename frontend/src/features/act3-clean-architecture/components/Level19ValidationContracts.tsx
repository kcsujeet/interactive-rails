...
export function Level19ValidationContracts({
	onComplete,
}: LevelComponentProps) {
	...
	const handleComplete = async () => {
		const success = await completeLevel('act3-level19-validation-contracts', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
...
export default Level19ValidationContracts;
