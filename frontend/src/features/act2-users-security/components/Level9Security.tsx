...
export function Level9Security({ onComplete, onExit }: LevelComponentProps) {
	...
	const securedSecrets = secrets.filter(
		(s) => s.location === s.correctLocation,
	);
	...
	const validateSolution = (): ValidationResult => {
		...
		const wrongLocation = secrets.filter(
			(s) =>
				s.location &&
				s.location !== 'exposed' &&
				s.location !== s.correctLocation,
		);
			...
			<LeftPanel>
				...
				>
					...
					<div className="p-4 border-t border-border">
						...
						<div
							className={`p-3 rounded-lg ${exposedSecrets.length > 0 ? 'bg-destructive/20 border border-destructive' : 'bg-success/20 border border-success'}`}
						>
							{exposedSecrets.length > 0 ? (
								...
								<div className="text-success text-sm">All secrets secured</div>
							)}
						</div>
					...
					<div className="p-4 border-t border-border">
						...
						<div className="h-2 bg-secondary rounded-full overflow-hidden">
							<div
								...
								style={{
									width: `${(securedSecrets.length / secrets.length) * 100}%`,
								}}
							/>
						</div>
					</div>
						...
						<div className="absolute inset-0 bg-destructive/80 flex items-center justify-center z-50 animate-pulse">
							<div className="text-center">
								...
								<div className="text-destructive-foreground mt-2">
									Hackers found your exposed credentials
								</div>
							</div>
						</div>
					)}
					...
					<div className="max-w-4xl mx-auto grid grid-cols-3 gap-6">
									...
									.map((secret) => (
										...
										>
											<div className="w-full">
												<div className="text-destructive/80 font-medium text-sm">
													{secret.name}
												</div>
												<div className="text-destructive font-mono text-xs truncate">
													{secret.value}
												</div>
											</div>
										</Button>
									))}
								{secrets.filter((s) => s.location === 'exposed').length ===
									0 && (
							...
							<div className="p-4 space-y-2 min-h-[300px]">
								...
								{secrets
									...
									.map((secret) => (
										...
										>
											<div
												className={`font-medium text-sm ${secret.correctLocation === 'env' ? 'text-success' : 'text-destructive'}`}
											>
												{secret.name}
											</div>
						...
						<div className="bg-card rounded-xl border-2 border-success overflow-hidden">
							...
							<div className="p-4 space-y-2 min-h-[300px]">
								...
								{secrets
									.filter((s) => s.location === 'credentials')
									.map((secret) => (
										<div
											className={`p-3 rounded-lg ${
										...
										>
											<div
												className={`font-medium text-sm ${secret.correctLocation === 'credentials' ? 'text-success' : 'text-destructive'}`}
											>
												{secret.name}
											</div>
											<div className="text-muted-foreground font-mono text-xs">
					...
					files={[
						{
							...
							language: 'yaml',
							code:
								secrets.find((s) => s.id === 'db_password')?.location ===
								'credentials'
									? `production:
  adapter: postgresql
						...
						<div className="space-y-2 text-xs">
							<div className="p-2 bg-warning/10 rounded border border-warning/50">
								...
								<div className="text-muted-foreground">
									Environment, URLs, feature flags
								</div>
							</div>
