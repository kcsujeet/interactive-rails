...
export function Level32MessageQueues({
	onComplete,
	onExit,
}: LevelComponentProps) {
							...
							setConsumers((c) =>
								c.map((consumer) =>
									consumer.subscribed
										? {
												...consumer,
												messagesHandled: consumer.messagesHandled + 1,
											}
										: consumer,
								),
							);
	...
	const validateSolution = (): ValidationResult => {
		...
		const totalHandled = consumers.reduce(
			(sum, c) => sum + c.messagesHandled,
			0,
		);
		if (totalHandled < 5) {
			return {
	...
	const handleComplete = async () => {
		const success = await completeLevel('act6-level32-message-queues', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
				...
				>
					...
					<div className="p-4 border-t border-border">
						<div className="grid grid-cols-2 gap-2">
							...
							<div className="bg-card p-2 rounded text-center">
								<div
									className={`text-xl font-bold ${deadLetterQueue.length > 0 ? 'text-destructive' : 'text-success'}`}
								>
					...
					<div className="p-4 border-t border-border">
						<div className="flex justify-between text-sm mb-2">
							...
							<span
								className={
									consumers.filter((c) => c.subscribed).length >= 2
										? 'text-success'
										: 'text-foreground'
								}
							>
								{consumers.filter((c) => c.subscribed).length} /{' '}
								{consumers.length}
							</span>
						</div>
					</div>
					...
					<div className="max-w-4xl mx-auto">
						...
						<div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
							<div className="bg-secondary px-4 py-3 border-b border-border">
								...
								<div className="text-xs text-muted-foreground">
									Click to publish an event to the queue
								</div>
							</div>
						...
						<div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
							<div className="bg-secondary px-4 py-3 border-b border-border flex justify-between items-center">
								...
								<span
									className={`w-2 h-2 rounded-full ${queueEnabled ? 'bg-success animate-pulse' : 'bg-muted-foreground'}`}
								/>
							</div>
							<div className="p-4 min-h-[100px]">
								{messages.length === 0 ? (
									<div className="text-center py-8 text-muted-foreground">
										{queueEnabled
											? 'Queue empty - publish some events!'
											: 'Enable queue to start'}
									</div>
								) : (
										...
										{messages.map((msg) => (
											<div
												className={`px-3 py-2 rounded-lg text-xs transition-all ${
													msg.status === 'queued'
														? 'bg-warning/40 text-warning border border-warning'
														: msg.status === 'processing'
															? 'bg-blue-900/40 text-blue-400 border border-blue-600 animate-pulse'
											...
											>
												...
												<div className="text-[10px] opacity-60">
													{msg.status}{' '}
													{msg.retries > 0 && `(retry ${msg.retries})`}
												</div>
											</div>
										))}
									</div>
							...
							<div className="p-4 grid grid-cols-4 gap-4">
								{consumers.map((consumer) => (
									...
									>
										...
										<div
											className={
												consumer.subscribed ? 'text-success' : 'text-foreground'
											}
										>
											{consumer.name}
										</div>
