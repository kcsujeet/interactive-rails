...
export function Level29Pagination({ onComplete }: LevelComponentProps) {
	...
	const handleComplete = async () => {
		const success = await completeLevel('act4-level29-pagination', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
...
export default Level29Pagination;
