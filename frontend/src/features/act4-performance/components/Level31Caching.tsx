...
export function Level31Caching({ onComplete }: LevelComponentProps) {
	...
	const handleComplete = async () => {
		const success = await completeLevel('act4-level31-caching', { stars: 3 });
		if (success) {
			onComplete({ stars: 3 });
		}
	};
