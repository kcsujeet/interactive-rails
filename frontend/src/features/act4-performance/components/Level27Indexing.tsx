...
export function Level27Indexing({ onComplete }: LevelComponentProps) {
	...
	const handleComplete = async () => {
		...
		const success = await completeLevel('act4-level27-database-indexing', {
			stars,
		});
		if (success) {
			onComplete({ stars });
...
export default Level27Indexing;
