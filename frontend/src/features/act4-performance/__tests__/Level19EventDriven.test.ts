...
describe('Level 19: Event-Driven Architecture', () => {
	describe('Initial State', () => {
		...
		test('should require processing order without event bus', () => {
			...
			expect(
				result.details!.some((d) => d.includes('WITHOUT the Event Bus')),
			).toBe(true);
		});
		...
		test('should require enabling event bus', () => {
			...
			expect(
				result.details!.some((d) => d.includes('Enable the Event Bus')),
			).toBe(true);
		});
		...
		test('should require processing order with event bus', () => {
			...
			expect(
				result.details!.some((d) => d.includes('WITH the Event Bus')),
			).toBe(true);
		});
	});
