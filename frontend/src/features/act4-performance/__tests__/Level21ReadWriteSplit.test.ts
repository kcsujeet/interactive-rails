...
function validateLevel21Solution(state: Level21State): ValidationResult {
	...
	if (state.primaryLoad >= 50) {
		errors.push(
			`Primary database load too high (${state.primaryLoad}%) - reads should go to replica`,
		);
	}

	if (state.readCount < 10) {
		errors.push(
			`Process at least 10 read queries (currently ${state.readCount})`,
		);
	}
...
describe('Level 21: Read/Write Split', () => {
	describe('Initial State', () => {
		...
		test('should require enabling replica', () => {
			...
			expect(result.details!.some((d) => d.includes('read replica'))).toBe(
				true,
			);
		});
		...
		test('should require low primary load', () => {
			...
			expect(result.details!.some((d) => d.includes('load too high'))).toBe(
				true,
			);
		});
		...
		test('should require processing reads', () => {
			...
			expect(result.details!.some((d) => d.includes('10 read queries'))).toBe(
				true,
			);
		});
	});
