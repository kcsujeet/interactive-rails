...
import type { BaseNode, Connection, ModelConfig } from '../../../shared/types/pipeline';
...
export function detectNPlusOne(
...
): { isNPlusOne: boolean; count: number; pattern?: string } {
  if (queries.length < 2) {
  ...
  for (const [tableName, tableQueries] of queryGroups) {
    ...
    if (selectQueries.length > 3) {
      ...
      const hasLoopIterations = selectQueries.some((q) => q.loopIteration !== undefined);
...
export function aggregateQueries(requests: SimulatedRequest[]): SimulatedQuery[] {
...
export function calculateStabilityTrend(
  ...
  const recentAvg =
    previousScores.slice(-5).reduce((a, b) => a + b, 0) / Math.min(5, previousScores.length);
  const olderAvg =
...
export function checkObjectiveMet(
  ...
  if (targetMetrics.maxQueriesPerRequest !== undefined) {
    if (metrics.queriesPerRequest > targetMetrics.maxQueriesPerRequest) return false;
  }
