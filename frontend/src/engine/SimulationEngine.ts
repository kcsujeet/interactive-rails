...
const ENEMY_STATS: Record<EnemyType, { hp: number; damage: number; speed: number }> = {
...
export function calculateStabilityTrend(history: number[]): 'improving' | 'stable' | 'degrading' {
...
export function detectNPlusOne(nodes: PipelineNode[], edges: PipelineEdge[]): number {
  let count = 0;
...
export function shouldSpawnEnemy(
  ...
  if (metrics.memory.pressure === 'high' || metrics.memory.pressure === 'critical') {
...
export function createEnemy(type: EnemyType, canvasWidth: number, canvasHeight: number): Enemy {
  const stats = ENEMY_STATS[type];
  const side = Math.floor(Math.random() * 4);

  let position: Position;
  switch (side) {
    case 0: position = { x: Math.random() * canvasWidth, y: 0 }; break;
    case 1: position = { x: canvasWidth, y: Math.random() * canvasHeight }; break;
    case 2: position = { x: Math.random() * canvasWidth, y: canvasHeight }; break;
    default: position = { x: 0, y: Math.random() * canvasHeight };
  }
...
export function updateEnemyPositions(enemies: Enemy[], targetX: number, targetY: number): Enemy[] {
...
export function processDefenseAttacks(
  ...
  const updatedDefenses = defenses.map((defense) => {
    if (defense.currentCooldown > 0) {
      return { ...defense, currentCooldown: defense.currentCooldown - 1, isActive: false };
    }
    ...
    for (let i = 0; i < updatedEnemies.length; i++) {
      ...
      if (dist <= defense.range) {
        ...
        updatedEnemies[i] = { ...enemy, hp: Math.max(0, newHp), isActive: newHp > 0 };
        return { ...defense, currentCooldown: defense.cooldown, isActive: true };
      }
    }
...
export function tick(state: SimulationState, config: SimulationConfig = DEFAULT_CONFIG): SimulationState {
  ...
  const newStabilityHistory = [...state.stabilityHistory, newStabilityScore].slice(-300);

  // Check objective
  const objectiveHoldTicks = newStabilityScore >= config.stabilityThreshold
    ? (state.objectiveProgress / 100) * config.stabilityDuration + 1
    : 0;
  const objectiveProgress = Math.min(100, (objectiveHoldTicks / config.stabilityDuration) * 100);
...
export function createSimulationRunner(initialState: SimulationState, config: SimulationConfig = DEFAULT_CONFIG) {
