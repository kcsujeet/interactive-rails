...
function validatePipeline(nodes: PipelineNode[], edges: PipelineEdge[]): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  for (const node of nodes) {
    ...
    if (!connectedNodeIds.has(node.id)) {
      errors.push(`Node "${node.data.label}" (${node.id.slice(0, 8)}) is not connected`);
    }
  }
    ...
    subscribeWithSelector(
      immer((set, get) => ({
        ...
        onNodesChange: (changes) => {
          set((state) => {
            state.nodes = applyNodeChanges(changes, state.nodes) as PipelineNode[];
          });
        },

        onEdgesChange: (changes) => {
          set((state) => {
            state.edges = applyEdgeChanges(changes, state.edges) as PipelineEdge[];
          });
        },
