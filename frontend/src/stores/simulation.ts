...
const DEFENSE_STATS: Record<DefenseType, { power: number; range: number; cooldown: number }> = {
...
function calculateStabilityTrend(history: number[]): 'improving' | 'stable' | 'degrading' {
      ...
      immer((set, get) => ({
        ...
        processTick: () => {
          ...
          set((state) => {
            ...
            state.stabilityTrend = calculateStabilityTrend(state.stabilityHistory);
            ...
            state.objectiveMet = state.objectiveHoldTicks >= state.config.stabilityDuration;
        ...
        recordMetricsSnapshot: () => {
          ...
          set((state) => {
            state.metricsHistory = [...metricsHistory.slice(-99), { ...metrics }];
          });
        },
        ...
        checkObjective: () => {
          ...
          set((state) => {
            ...
            state.objectiveMet = state.objectiveHoldTicks >= config.stabilityDuration;
          });
        },
      }))
    ),
