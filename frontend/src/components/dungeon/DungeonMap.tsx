// Dungeon map showing room grid with progress
...
export function DungeonMap({
  ...
  const allRooms = [...dungeon.rooms, ...(dungeon.bossRoom ? [dungeon.bossRoom] : [])];
    ...
    <div className={`bg-gray-800 rounded-lg p-4 ${className}`}>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-white">{dungeon.name}</h2>
      ...
      <div className="flex flex-wrap items-center justify-center gap-y-4">
        {allRooms.map((room, index) => {
          ...
          const isBoss = index === dungeon.rooms.length && dungeon.bossRoom !== undefined;
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <div className="flex gap-3">
            <span className="text-gray-300">
              1 = {dungeon.starThresholds.one}
            </span>
            <span className="text-gray-300">
              2 = {dungeon.starThresholds.two}
            </span>
            <span className="text-amber-300">
              3 = {dungeon.starThresholds.three}
            </span>
          </div>
        </div>
      </div>
...
// Dungeon selection card for dungeon list
export function DungeonCard({
  ...
  const totalRooms = dungeon.rooms.length + (dungeon.bossRoom ? 1 : 0);
