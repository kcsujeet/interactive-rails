...
export function LevelPlayApp({ levelId }: LevelPlayAppProps) {
  ...
  const { isPipelineBroken, breakReason, hasSolutionNode, checkChallenge } = validation;
  ...
  function loadLevel() {
    ...
    const info = activeLevel
      ? {
          ...
          goal: activeLevel.problem.goal || 'Fix the pipeline to complete this level.',
        }
      : challenge
        ? {
            name: challenge.name,
  ...
  function startLevel() {
    if (challenge) {
      ...
      for (const connDef of challenge.initialConnections) {
        const sourceNode = challenge.initialNodes.find((n) => n.type === connDef.sourceType);
        const targetNode = challenge.initialNodes.find((n) => n.type === connDef.targetType);
      ...
      const initialConns: Connection[] = level.startingPipeline.connections.map((conn) => ({
        id: conn.id,
  ...
  async function completeLevel(stars: number) {
    ...
    try {
      ...
      let stackChoices = undefined;

      if (terminals.length > 0) {
        // Find what's connected to the terminal
        const connectedIds = new Set<string>();
        connections.forEach((c) => {
          if (c.sourceNodeId === terminals[0].id) connectedIds.add(c.targetNodeId);
          if (c.targetNodeId === terminals[0].id) connectedIds.add(c.sourceNodeId);
        });
...
if ((gameState === 'playing' || gameState === 'paused') && CustomLevelComponent) {
  return (
  ...
  return (
      <div className="h-full flex">
        <NodePalette
          ...
          showMetrics={
            !!challenge?.initialMetrics || (!!level && !!getActForLevel(levelId)?.metricsVisible)
          }
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 relative flex flex-col">
            ...
            {gameState === 'paused' && <PausedOverlay onResume={resumeLevel} onExit={exitLevel} />}
          </div>
        </div>
