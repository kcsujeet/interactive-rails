...
import { type DragEvent, type MouseEvent, type RefObject, useRef, useState } from 'react';
...
export function usePipelineState(): UsePipelineStateReturn {
  ...
  function completeConnection(e: MouseEvent, targetNodeId: string) {
    ...
    const exists = connections.some(
      (c) =>
        (c.sourceNodeId === pendingConnection.sourceNodeId && c.targetNodeId === targetNodeId) ||
        (c.sourceNodeId === targetNodeId && c.targetNodeId === pendingConnection.sourceNodeId)
    );
    ...
    if (!exists) {
      ...
      if (sourceNode?.type === 'terminal' || targetNode?.type === 'terminal') {
        const otherNode = sourceNode?.type === 'terminal' ? targetNode : sourceNode;
        if (otherNode) {
  ...
  function updateNode(nodeId: string, updates: Partial<PlacedNode>) {
    setPlacedNodes((prev) => prev.map((n) => (n.id === nodeId ? { ...n, ...updates } : n)));
  }
  ...
  function deleteSelectedNode() {
    ...
    setConnections((prev) =>
      prev.filter((c) => c.sourceNodeId !== selectedNodeId && c.targetNodeId !== selectedNodeId)
    );
    setSelectedNodeId(null);
  }
