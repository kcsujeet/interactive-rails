...
import { NODE_BEHAVIORS, isConnectionAllowed } from '../../engine/nodeBehavior';
import type { LevelChallenge, NodeTypeInfo } from './types';
...
export const nodeTypes: NodeTypeInfo[] = [
  ...
  { type: 'react', name: 'React', color: '#61dafb', icon: '⚛️', description: 'React Frontend (API mode)' },
  { type: 'hotwire', name: 'Hotwire', color: '#ff6b6b', icon: '⚡', description: 'Hotwire/Turbo (monolithic)' },
  { type: 'erb', name: 'ERB', color: '#cc0000', icon: '💎', description: 'Rails ERB Views' },
  {
    type: 'request',
    name: 'Request',
    color: '#3b82f6',
  ...
  { type: 'model', name: 'Model', color: '#f59e0b', icon: 'M', description: 'ActiveRecord model' },
  {
    type: 'database',
    name: 'Database',
    color: '#ef4444',
  ...
  { type: 'view', name: 'View', color: '#a855f7', icon: 'V', description: 'ERB template' },
  { type: 'response', name: 'Response', color: '#22c55e', icon: '<', description: 'HTTP response' },
  ...
  { type: 'index', name: 'Index', color: '#f97316', icon: 'I', description: 'Database index' },
  { type: 'service', name: 'Service', color: '#8b5cf6', icon: 'S', description: 'Service object (PORO)' },
  { type: 'command', name: 'Command', color: '#7c3aed', icon: '!', description: 'Atomic command operation' },
  { type: 'contract', name: 'Contract', color: '#06b6d4', icon: '📋', description: 'dry-validation schema' },
  { type: 'form', name: 'Form', color: '#14b8a6', icon: '📝', description: 'Form object (multi-model)' },
  { type: 'component', name: 'Component', color: '#f472b6', icon: '🧩', description: 'ViewComponent' },
  { type: 'batch', name: 'Batch', color: '#06b6d4', icon: 'B', description: 'Bulk operations' },
  { type: 'scope', name: 'Scope', color: '#84cc16', icon: 's', description: 'Named query scope' },

  // Act III - Scaling
  { type: 'cache', name: 'Cache', color: '#06b6d4', icon: '$', description: 'Rails.cache' },
  {
    type: 'fragment_cache',
    name: 'Fragment Cache',
    color: '#0ea5e9',
  ...
  { type: 'worker', name: 'Worker', color: '#9333ea', icon: 'W', description: 'Sidekiq worker' },
  { type: 'mailer', name: 'Mailer', color: '#ec4899', icon: '@', description: 'ActionMailer' },
  { type: 'storage', name: 'Storage', color: '#f472b6', icon: '☁️', description: 'S3/ActiveStorage' },
  { type: 'webhook_endpoint', name: 'Webhook', color: '#10b981', icon: '🔔', description: 'Incoming webhook endpoint' },
  { type: 'session', name: 'Session', color: '#a855f7', icon: 'z', description: 'Session storage' },
  { type: 'env', name: 'ENV', color: '#374151', icon: '🔒', description: 'Environment Variables' },
  ...
  { type: 'redis', name: 'Redis', color: '#dc2626', icon: 'r', description: 'Redis server' },
  { type: 'pubsub', name: 'Pub/Sub', color: '#f43f5e', icon: '~', description: 'Redis Pub/Sub' },
  { type: 'websocket', name: 'WebSocket', color: '#e11d48', icon: 'w', description: 'ActionCable' },
  {
    type: 'event_bus',
    name: 'Event Bus',
    color: '#be185d',
  ...
  { type: 'cdn', name: 'CDN', color: '#4f46e5', icon: 'N', description: 'Content delivery' },
  {
    type: 'elasticsearch',
    name: 'Elasticsearch',
    color: '#fcd34d',
...
export function getNodeInfo(type: string): NodeTypeInfo {
  return nodeTypes.find((n) => n.type === type) || { type, name: type, color: '#6b7280' };
}
...
export function getConnectionError(sourceType: string, targetType: string): string | null {
  const result = isConnectionAllowed(sourceType, targetType);
...
export const levelChallenges: Record<string, LevelChallenge> = {
  ...
  @posts.each do |post|
    ...
    successCondition: (nodes, connections) => {
      ...
      if (hasEagerLoad && eagerLoadConnected) {
        return { success: true, message: 'N+1 fixed! Queries reduced from 101 to 2.' };
      }
      return {
        success: false,
        message:
  ...
  'tutorial-indexing': {
    ...
    scenario: "User lookup by email is taking 3 seconds because there's no index.",
  ...
  # Scans 1,000,000 rows = 3000ms`,
    ...
    successCondition: (nodes, connections) => {
      ...
      if (hasIndex && indexConnected) {
        return { success: true, message: 'Index added! Query time reduced from 3000ms to 5ms.' };
      }
      return {
        success: false,
        message: 'Latency is still too high. The query is scanning too many rows.',
      };
    },
  ...
  @posts = Post.includes(:author, :comments)
    ...
    successCondition: (nodes) => {
      ...
      if (hasCache) {
        return { success: true, message: 'Caching added! Subsequent requests will be instant.' };
      }
      return {
        success: false,
        message:
  ...
  - Missing eager loading`,
    ...
    successCondition: (nodes) => {
      ...
      if (cacheCount >= 2) {
        return { success: true, message: 'The Database Guardian is defeated! All issues fixed.' };
      }
      return {
        success: false,
        message: 'Performance is still poor. Multiple optimizations are needed.',
      };
    },
