...
export function useLevelCompletion(): UseLevelCompletionReturn {
  ...
  const completeLevel = useCallback(async (levelId: string, data: CompletionData): Promise<boolean> => {
    ...
    try {
      ...
      if (data.stackChoices) {
        const existingChoices = localStorage.getItem('rails-expert-game-choices');
        ...
        const updatedChoices = {
          ...
          constraints: {
            apiOnly: (data.stackChoices.frontend || choices.frontend) === 'react',
            canShard: (data.stackChoices.database || choices.database) === 'postgresql',
          },
        };

        localStorage.setItem('rails-expert-game-choices', JSON.stringify(updatedChoices));
      }

      // Save any decisions to localStorage
      if (data.decisions) {
        const existingDecisions = localStorage.getItem('rails-expert-level-decisions');
        const decisions = existingDecisions ? JSON.parse(existingDecisions) : {};
        ...
        localStorage.setItem('rails-expert-level-decisions', JSON.stringify(updatedDecisions));
      }
      ...
      setCompletionError(err instanceof Error ? err.message : 'Failed to save completion');
      return false;
    } finally {
