...
export function ParticleCanvas({
  ...
  const poofEffects = useRef<Map<string, { x: number; y: number; progress: number }>>(new Map());
  ...
  const draw = useCallback(() => {
    ...
    for (const particle of particles) {
      ...
      if (visual.trail && particle.progress > 0.1) {
        ...
        const trailStartX = particle.x + (particle.targetX - particle.x) * Math.max(0, particle.progress - 0.1);
        const trailStartY = particle.y + (particle.targetY - particle.y) * Math.max(0, particle.progress - 0.1);
      ...
      if (visual.pulse) {
        ...
        const pulseSize = visual.size * (1 + 0.3 * Math.sin(pulsePhase * Math.PI * 2));
  ...
  // Handle poof effect
  const triggerPoof = useCallback((id: string, x: number, y: number) => {
