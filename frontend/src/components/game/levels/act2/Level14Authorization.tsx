...
export function Level14Authorization({
	onComplete,
	onExit,
}: LevelComponentProps) {
	...
	const correctRules = rules.filter(
		(r) =>
	...
	const validateSolution = (): ValidationResult => {
		...
		for (const rule of rules) {
			const missing = rule.allowedRoles.filter(
				(ar) => !rule.selectedRoles.includes(ar),
			);
			const extra = rule.selectedRoles.filter(
				(sr) => !rule.allowedRoles.includes(sr),
			);
			...
			if (extra.length > 0) {
				errors.push(
					`${rule.action}: ${extra.join(', ')} shouldn't have access`,
				);
			}
		}
		...
		if (errors.length > 0) {
			return {
				valid: false,
				message: 'Policy needs adjustment!',
	...
	const handleComplete = async () => {
		const success = await completeLevel('act2-level14-authorization', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
					...
					<div className="p-4 border-t border-border">
						...
						<div className="h-2 bg-secondary rounded-full overflow-hidden">
							<div
								...
								style={{
									width: `${(correctRules.length / rules.length) * 100}%`,
								}}
							/>
						</div>
					</div>
						...
						<div className="bg-card rounded-xl border border-border overflow-hidden">
							<table className="w-full">
								...
								<tbody>
									{rules.map((rule) => {
										const isCorrect =
											rule.selectedRoles.length === rule.allowedRoles.length &&
											rule.selectedRoles.every((sr) =>
												rule.allowedRoles.includes(sr),
											);
										...
										return (
											<tr className="border-t border-border" key={rule.id}>
												...
												{ROLES.map((role) => {
													const isSelected = rule.selectedRoles.includes(
														role.id,
													);
													const shouldBeSelected = rule.allowedRoles.includes(
														role.id,
													);
													...
													return (
														<td className="px-4 py-3 text-center" key={role.id}>
															<Button
																...
																onClick={() =>
																	toggleRoleForRule(rule.id, role.id)
																}
																size="icon"
																variant="outline"
															>
