...
export function Level13FormObjects({
	onComplete,
	onExit,
}: LevelComponentProps) {
	...
	const handleComplete = async () => {
		const success = await completeLevel('act2-level13-form-objects', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
							...
							{fields
								...
								.map((field) => (
									<Button
										...
										style={{
											borderColor: getModelColor(field.model),
											backgroundColor: `${getModelColor(field.model)}10`,
										}}
										variant="outline"
									>
										<div className="flex items-center justify-between w-full">
											<span
												...
												style={{ color: getModelColor(field.model) }}
											>
												{field.name}
											</span>
