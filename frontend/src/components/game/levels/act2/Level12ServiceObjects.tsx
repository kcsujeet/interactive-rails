...
const SERVICE_COMPONENTS: ServiceComponent[] = [
  { id: 'order_dep', name: '@order', code: 'attr_reader :order', color: '#3b82f6', description: 'Order to process', correctSection: 'dependencies', currentSection: null },
  { id: 'gateway_dep', name: '@gateway', code: 'attr_reader :gateway', color: '#8b5cf6', description: 'Payment gateway', correctSection: 'dependencies', currentSection: null },
  { id: 'init', name: 'initialize', code: 'def initialize(order, gateway: Stripe)', color: '#06b6d4', description: 'Dependency injection', correctSection: 'initialize', currentSection: null },
  { id: 'validate', name: 'validate!', code: 'raise InvalidOrder unless order.valid?', color: '#22c55e', description: 'Check preconditions', correctSection: 'call', currentSection: null },
  { id: 'charge', name: 'charge_card', code: 'gateway.charge(order.total)', color: '#f59e0b', description: 'Process payment', correctSection: 'call', currentSection: null },
  { id: 'notify', name: 'send_receipt', code: 'OrderMailer.receipt(order).deliver_later', color: '#ef4444', description: 'Send confirmation', correctSection: 'call', currentSection: null },
  { id: 'log', name: 'log_success', code: 'Rails.logger.info("Order #{order.id} completed")', color: '#10b981', description: 'Audit logging', correctSection: 'private', currentSection: null },
];

const SECTIONS = [
  { id: 'dependencies', name: 'Dependencies', description: 'What the service needs' },
  { id: 'initialize', name: 'Initialize', description: 'Setup with dependency injection' },
  { id: 'call', name: 'Call Method', description: 'The main logic steps' },
  { id: 'private', name: 'Private Helpers', description: 'Supporting methods' },
];

export function Level12ServiceObjects({ onComplete, onExit }: LevelComponentProps) {
  const { completeLevel } = useLevelCompletion();
  const [components, setComponents] = useState<ServiceComponent[]>(SERVICE_COMPONENTS);
  const [draggedComponent, setDraggedComponent] = useState<string | null>(null);
  const [dragOverSection, setDragOverSection] = useState<string | null>(null);

  const correctlyPlaced = components.filter(c => c.currentSection === c.correctSection);
  ...
  const validateSolution = (): ValidationResult => {
    ...
    const wrongSection = components.filter(c => c.currentSection && c.currentSection !== c.correctSection);
    if (wrongSection.length > 0) {
      errors.push(`${wrongSection.length} component(s) in wrong section`);
    }

    if (errors.length > 0) {
      return { valid: false, message: 'Service structure needs work!', details: errors };
    }

    return { valid: true, message: 'Well-structured service object!' };
  };

  const handleDragStart = (componentId: string) => setDraggedComponent(componentId);
  const handleDragEnd = () => { setDraggedComponent(null); setDragOverSection(null); };

  const handleDropOnSection = (sectionId: string) => {
    if (!draggedComponent) return;
    setComponents(prev => prev.map(c => c.id === draggedComponent ? { ...c, currentSection: sectionId } : c));
    setDraggedComponent(null);
    setDragOverSection(null);
  };

  const handleRemove = (componentId: string) => {
    setComponents(prev => prev.map(c => c.id === componentId ? { ...c, currentSection: null } : c));
  };

  const handleComplete = async () => {
    const success = await completeLevel('act2-level12-service-objects', { stars: 3 });
    if (success) {
      onComplete({ stars: 3 });
    }
  };

  const getComponentsForSection = (sectionId: string) => components.filter(c => c.currentSection === sectionId);
  const paletteComponents = components.filter(c => !c.currentSection);

  const generateServiceCode = () => {
    const deps = getComponentsForSection('dependencies').map(c => `  ${c.code}`).join('\n');
    const init = getComponentsForSection('initialize').map(c => `  ${c.code}\n    @order = order\n    @gateway = gateway\n  end`).join('\n');
