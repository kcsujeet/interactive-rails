...
export function Level7Services({ onComplete, onExit }: LevelComponentProps) {
  ...
  const [components, setComponents] = useState<ServiceComponent[]>(INITIAL_COMPONENTS);
  const [placedInSection, setPlacedInSection] = useState<Record<string, string[]>>({
    dependencies: [],
    initialize: [],
    steps: [],
    results: [],
  ...
  const validateSolution = (): ValidationResult => {
    ...
    for (const section of SERVICE_SECTIONS) {
      ...
      if (placed.length < section.requiredCount) {
        errors.push(`${section.name} needs ${section.requiredCount - placed.length} more component(s)`);
      }
    }
    ...
    for (let i = 0; i < stepsWithOrder.length - 1; i++) {
      ...
      if (current && next && current.order! > next.order!) {
        errors.push(`Steps are in wrong order: ${current.name} should come before ${next.name}`);
      }
    }
  ...
  const handleRemoveFromSection = (componentId: string) => {
    ...
    setComponents(prev =>
      prev.map(c =>
        c.id === componentId ? { ...c, placed: false } : c
      )
    );
  };
  ...
  // Calculate progress
  const totalRequired = SERVICE_SECTIONS.reduce((sum, s) => sum + s.requiredCount, 0);
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-card rounded-xl border border-border overflow-hidden">
              ...
              {SERVICE_SECTIONS.map(section => {
                ...
                const isComplete = sectionComponents.length >= section.requiredCount;
                const allCorrect = sectionComponents.every(c => c.section === section.id);
                      ...
                      {sectionComponents.length > 0 ? (
                        <div className="space-y-2">
                          {sectionComponents.map(comp => {
                            const isCorrectSection = comp.section === section.id;
                            return (
                              <div
                                key={comp.id}
