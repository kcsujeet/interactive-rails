...
export function Level11Authorization({ onComplete, onExit }: LevelComponentProps) {
  ...
  const validateSolution = (): ValidationResult => {
    ...
    if (breachOccurred) {
      errors.push('A security breach occurred! Reset and try again with the Policy enabled');
    }

    if (blockedCount < 3) {
      errors.push(`Need to block at least 3 unauthorized requests (currently ${blockedCount})`);
    }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setRequests(prev => [...prev.slice(-12), {
        id,
        x: 50,
    ...
    const interval = setInterval(() => {
      setRequests(prev => prev.map(r => {
        ...
        if (policyAdded && newX >= 300 && newX < 310 && !r.blocked) {
          ...
          const shouldBlock = r.type === 'hacker' || (r.action === 'delete' && r.type !== 'admin');
        ...
        if (!policyAdded && newX >= 450 && r.type === 'hacker' && !r.blocked) {
  ...
  const handleComplete = async () => {
    const success = await completeLevel('act2-level11-authorization', { stars: 3 });
    if (success) {
      onComplete({ stars: 3 });
    }
  };
      ...
      <CenterPanel>
        ...
        <div className="flex-1 relative bg-background overflow-hidden">
          ...
          <svg className="absolute inset-0 w-full h-full">
            ...
            <text x="80" y="255" textAnchor="middle" fill="#9ca3af" fontSize="12">Controller</text>

            {/* Policy (if added) */}
            {policyAdded && (
              <>
                <rect x="280" y="180" width="60" height="140" fill="#8b5cf6" rx="8" />
                <text x="310" y="255" textAnchor="middle" fill="white" fontSize="11">Policy</text>
                <text x="310" y="270" textAnchor="middle" fill="white" fontSize="9">authorize!</text>
              </>
            )}

            {/* Service */}
            <rect x="450" y="150" width="100" height="200" fill="#374151" rx="8" />
            <text x="500" y="255" textAnchor="middle" fill="#9ca3af" fontSize="12">Service</text>

            {/* Connection lines */}
            <line x1="120" y1="250" x2={policyAdded ? "280" : "450"} y2="250" stroke="#4b5563" strokeWidth="2" strokeDasharray="5,5" />
            {policyAdded && (
              <line x1="340" y1="250" x2="450" y2="250" stroke="#8b5cf6" strokeWidth="2" />
            )}
            ...
            {/* Block effects */}
            {requests.filter(r => r.blocked && r.x >= 300 && r.x < 360).map(r => (
              <g key={`block-${r.id}`}>
                <text x={r.x} y={r.y - 20} textAnchor="middle" fill="#ef4444" fontSize="10" fontWeight="bold">
                  DENIED
                </text>
              </g>
            ))}
