...
export function Level14BackgroundJobs({ onComplete, onExit }: LevelComponentProps) {
  ...
  const validateSolution = (): ValidationResult => {
    ...
    if (!experiencedBlocking) {
      errors.push('Generate a PDF WITHOUT the worker first to see the blocking behavior');
    }
    ...
    if (completedJobs < 2) {
      errors.push(`Complete at least 2 background jobs (currently ${completedJobs})`);
    }

    if (workerEnabled && requestTime !== null && requestTime > 500) {
      errors.push('Response time too slow - background jobs should return instantly');
    }
  ...
  const handleComplete = async () => {
    const success = await completeLevel('act3-level14-background-jobs', { stars: 3 });
    if (success) {
      onComplete({ stars: 3 });
    }
  };
            ...
            {workerEnabled && (
              <>
                ...
                <div className="bg-purple-900/40 border border-purple-600 rounded-xl p-4 w-48">
                  <div className="text-purple-400 font-mono text-sm mb-2">Sidekiq Worker</div>
