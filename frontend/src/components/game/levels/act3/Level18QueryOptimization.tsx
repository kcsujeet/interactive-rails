...
const INITIAL_CHALLENGES: QueryChallenge[] = [
		...
		techniques: [
			{
				id: 'select',
				name: 'select(:columns)',
				description: 'Only load needed columns',
				applied: false,
			},
		],
	},
		...
		techniques: [
			{
				id: 'pluck',
				name: 'pluck(:column)',
				description: 'Return raw array, skip AR',
				applied: false,
			},
		],
	},
	...
	},
	{
		...
		techniques: [
			{
				id: 'count',
				name: 'count vs length',
				description: 'Let DB count, not Ruby',
				applied: false,
			},
		],
	},
		...
		techniques: [
			{
				id: 'exists',
				name: 'exists?',
				description: 'Check existence without loading',
				applied: false,
			},
		],
	},
...
export function Level18QueryOptimization({
	onComplete,
	onExit,
}: LevelComponentProps) {
	const { completeLevel } = useLevelCompletion();
	const [challenges, setChallenges] =
		useState<QueryChallenge[]>(INITIAL_CHALLENGES);
	...
	const handleComplete = async () => {
		const success = await completeLevel('act3-level18-query-optimization', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
						...
						<div className="space-y-2">
							{challenges.map((challenge) => (
								<Button
									...
									variant={
										activeChallenge === challenge.id ? 'default' : 'outline'
									}
								>
									<div className="flex justify-between items-center w-full">
										<span
											className={`text-sm ${challenge.solved ? 'text-success' : 'text-foreground'}`}
										>
											{challenge.title}
										</span>
										{challenge.solved && (
					...
					<div className="max-w-3xl mx-auto">
						...
						<div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
							...
							<div className="grid grid-cols-2 gap-4 p-4">
								{/* Before */}
								<div
									className={`rounded-lg border-2 p-4 ${currentChallenge.solved ? 'border-border opacity-50' : 'border-destructive bg-destructive/10'}`}
								>
									<div className="text-xs text-destructive font-semibold uppercase mb-2">
										Before (Slow)
									</div>
									...
									<div className="grid grid-cols-2 gap-2 text-xs">
										<div className="bg-secondary p-2 rounded">
											...
											<div className="text-destructive font-bold">
												{currentChallenge.memoryBefore} MB
											</div>
										</div>
										<div className="bg-secondary p-2 rounded">
											<div className="text-muted-foreground">Time</div>
											<div className="text-destructive font-bold">
												{currentChallenge.timeBefore} ms
											</div>
										</div>
									</div>
								</div>
								...
								{/* After */}
								<div
									className={`rounded-lg border-2 p-4 ${currentChallenge.solved ? 'border-success bg-success/10' : 'border-border opacity-50'}`}
								>
									<div className="text-xs text-success font-semibold uppercase mb-2">
										After (Optimized)
									</div>
									<pre className="text-sm text-muted-foreground bg-secondary p-3 rounded mb-3 overflow-x-auto">
									...
									<div className="grid grid-cols-2 gap-2 text-xs">
										<div className="bg-secondary p-2 rounded">
											...
											<div className="text-success font-bold">
												{currentChallenge.memoryAfter} MB
											</div>
										</div>
										<div className="bg-secondary p-2 rounded">
											<div className="text-muted-foreground">Time</div>
											<div className="text-success font-bold">
												{currentChallenge.timeAfter} ms
											</div>
										</div>
									</div>
								</div>
							...
							<div className="px-4 pb-4">
								<div
									className={`text-center p-3 rounded-lg ${currentChallenge.solved ? 'bg-success/20' : 'bg-secondary'}`}
								>
									<span
										className={`text-sm font-medium ${currentChallenge.solved ? 'text-success' : 'text-muted-foreground'}`}
									>
							...
							<div className="p-4 space-y-3">
								{currentChallenge.techniques.map((technique) => (
									<Button
										...
										onClick={() =>
											!technique.applied &&
											applyTechnique(currentChallenge.id, technique.id)
										}
										variant={technique.applied ? 'default' : 'outline'}
									>
										<div className="flex justify-between items-center w-full">
											<span
												className={`font-mono text-sm ${technique.applied ? 'text-success' : 'text-primary'}`}
											>
												{technique.name}
											</span>
											{technique.applied && (
						...
						{currentChallenge.solved && (
							<div className="mt-6 bg-success/20 border border-success rounded-xl p-4 text-center">
								...
								<div className="text-sm text-muted-foreground">
									Memory: {currentChallenge.memoryBefore} MB →{' '}
									{currentChallenge.memoryAfter} MB | Time:{' '}
									{currentChallenge.timeBefore} ms →{' '}
									{currentChallenge.timeAfter} ms
								</div>
							</div>
