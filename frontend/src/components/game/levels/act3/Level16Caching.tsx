    ...
    <LevelLayout>
      <LeftPanel>
        ...
        >
          ...
          <div className="p-4 border-t border-border">
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Cache Stats</div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-success/20 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-success">{cacheHits}</div>
                <div className="text-xs text-success/70">Cache Hits</div>
              </div>
              <div className="bg-destructive/20 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-destructive">{cacheMisses}</div>
        ...
        <div className="flex-1 relative bg-background p-8">
          ...
          <div className="flex items-center justify-center gap-6 mb-8">
            ...
            <div className={`border rounded-xl p-4 w-40 text-center transition-colors ${
              dbLoad > 80 ? 'bg-destructive/20 border-destructive' :
              dbLoad > 50 ? 'bg-warning/20 border-warning' :
              'bg-card border-border'
            }`}>
              <div className="text-2xl mb-2">DB</div>
              <div className={`text-sm ${
                dbLoad > 80 ? 'text-destructive' : dbLoad > 50 ? 'text-warning' : 'text-muted-foreground'
              }`}>
                PostgreSQL
              </div>
              <div className="mt-2">
                <div className="text-xs text-muted-foreground mb-1">CPU: {dbLoad}%</div>
                <div className="bg-secondary rounded-full h-2 overflow-hidden">
                  <div
                    className={`h-full transition-all ${
                      dbLoad > 80 ? 'bg-destructive' : dbLoad > 50 ? 'bg-warning' : 'bg-success'
                    }`}
                    style={{ width: `${dbLoad}%` }}
                  />
                </div>
          ...
          <div className="bg-card rounded-xl p-4 max-w-2xl mx-auto">
            <div className="text-muted-foreground text-xs uppercase tracking-wider mb-3">Query Log</div>
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {queries.map(q => (
                <div
                  key={q.id}
                  className={`flex items-center justify-between p-2 rounded-lg ${
                    q.status === 'pending' ? 'bg-secondary' :
                    q.status === 'cache_hit' ? 'bg-success/20' :
                    'bg-destructive/20'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${
                      q.status === 'pending' ? 'bg-muted-foreground animate-pulse' :
                      q.status === 'cache_hit' ? 'bg-success' :
                      'bg-destructive'
                    }`} />
                    <span className="text-muted-foreground font-mono text-sm">GET /{q.type}</span>
                  </div>
                  <div className="text-sm">
                    {q.status === 'pending' && (
                    ...
                    {q.status === 'cache_hit' && (
                      <span className="text-success">{q.latency.toFixed(1)}ms (cache)</span>
                    )}
                    {q.status === 'cache_miss' && (
                      <span className="text-destructive">{q.latency.toFixed(1)}ms (db)</span>
                    )}
                  </div>
                </div>
              ))}
