...
const COMMANDS = [
  { cmd: 'Post.create(title: "Hello World", body: "My first post!")', operation: 'create', description: 'Create a new post' },
  { cmd: 'Post.all', operation: 'read-all', description: 'Get all posts' },
  { cmd: 'Post.find(1)', operation: 'read-one', description: 'Find post by ID' },
  { cmd: 'Post.first.update(title: "Updated Title")', operation: 'update', description: 'Update a post' },
  { cmd: 'Post.last.destroy', operation: 'destroy', description: 'Delete a post' },
];

export function Level3CRUD({ onComplete, onExit }: LevelComponentProps) {
  const { completeLevel } = useLevelCompletion();
  const [consoleHistory, setConsoleHistory] = useState<ConsoleEntry[]>([
    { id: 0, input: 'rails console', output: 'Loading development environment (Rails 7.1.0)', type: 'info' },
  ]);
  ...
  const validateSolution = (): ValidationResult => {
    ...
    if (missing.length > 0) {
      ...
      errors.push(`Try these operations: ${missing.map(m => opNames[m]).join(', ')}`);
    }
  ...
  const executeCommand = (cmd: string, operation: string) => {
    ...
    setConsoleHistory(prev => [...prev, { id: entryId, input: cmd, output, type }]);
  ...
  const handleComplete = async () => {
    const success = await completeLevel('act1-level3-crud-operations', { stars: 3 });
    if (success) {
      onComplete({ stars: 3 });
    }
  };
...
Post.destroy_all   # Delete all`,
            highlight: completedOperations.has('create') ? [2, 3] :
                       completedOperations.has('read-all') ? [6] :
                       completedOperations.has('read-one') ? [7] :
                       completedOperations.has('update') ? [14, 15, 16, 17] :
                       completedOperations.has('destroy') ? [20, 21] : [],
          }]}
