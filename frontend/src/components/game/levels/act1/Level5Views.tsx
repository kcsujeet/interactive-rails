...
const INITIAL_SLOTS: ERBSlot[] = [
  { id: 'title', type: 'output', content: null, correctContent: '@post.title', hint: 'Display the title' },
  { id: 'body', type: 'output', content: null, correctContent: '@post.body', hint: 'Display the body' },
  { id: 'loop', type: 'execute', content: null, correctContent: '@posts.each do |post|', hint: 'Loop through posts' },
  { id: 'link', type: 'output', content: null, correctContent: 'link_to post.title, post', hint: 'Link to post' },
];

const AVAILABLE_TAGS: ERBTag[] = [
  { id: 'post-title', content: '@post.title', type: 'output', description: 'Current post title' },
  { id: 'post-body', content: '@post.body', type: 'output', description: 'Current post body' },
  { id: 'posts-each', content: '@posts.each do |post|', type: 'execute', description: 'Loop through posts' },
  { id: 'link-to', content: 'link_to post.title, post', type: 'output', description: 'Link to a post' },
];

// Sample data to show in preview
const SAMPLE_POST = { id: 1, title: 'Welcome to Rails!', body: 'This is your first post. Rails makes web development fun!' };
const SAMPLE_POSTS = [
...
export function Level5Views({ onComplete, onExit }: LevelComponentProps) {
  ...
  const correctCount = slots.filter(s => s.content === s.correctContent).length;
  ...
  const validateSolution = (): ValidationResult => {
    ...
    const incorrectSlots = slots.filter(s => s.content && s.content !== s.correctContent);
  ...
  const handleDrop = (slotId: string) => {
    if (selectedTag) {
      ...
      if (tag) {
        setSlots(prev =>
          prev.map(s => (s.id === slotId ? { ...s, content: tag.content } : s))
        );
      }
    }
    setSelectedTag(null);
  ...
  const handleComplete = async () => {
    const success = await completeLevel('act1-level5-views-templates', { stars: 3 });
    if (success) {
      onComplete({ stars: 3 });
    }
  };
...
<%= link_to "Back", posts_path %>`,
              highlight: showSlots.filter(s => s.content === s.correctContent).map((_, i) => i * 2 + 1),
            },
            {
...
<%= link_to "New Post", new_post_path %>`,
              highlight: indexSlots.filter(s => s.content === s.correctContent).map(() => 4),
            },
          ]}
