...
const NODE_DEFS: Record<string, { name: string; icon: string; color: string; description: string }> = {
  ...
  controller: { name: 'Controller', icon: 'C', color: '#3b82f6', description: 'Controller' },
  ...
  response: { name: 'Response', icon: 'R', color: '#10b981', description: 'Response' },
  env: { name: 'ENV', icon: 'E', color: '#eab308', description: 'Environment variables' },
};
...
export function Level5Security({ onComplete, onExit }: LevelComponentProps) {
  ...
  const [credentialType, setCredentialType] = useState<'public' | 'encrypted' | null>(null);
...
production:
  ...
  database: myapp_production
  # Missing credentials!`,
      highlight: credentialType === 'public' ? [1, 6] : credentialType === 'encrypted' ? [1, 5, 6] : [4],
    });
...
# Set RAILS_MASTER_KEY in your CI/CD environment`,
      highlight: !databaseUnlocked ? [2, 3] : credentialType === 'public' ? [2, 3] : [2],
    });
      ...
      <CenterPanel>
        ...
        >
          ...
          {placedNodes.map(node => {
            ...
            const isLocked = node.id !== 'env-1' && !(isDatabase && envAdded && !credentialType);
