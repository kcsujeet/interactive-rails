...
export function Level22Sharding({ onComplete, onExit }: LevelComponentProps) {
  ...
  const handleComplete = async () => {
    const success = await completeLevel('act4-level22-sharding', { stars: 3 });
    if (success) {
      onComplete({ stars: 3 });
    }
  };
...
export default Level22Sharding;
