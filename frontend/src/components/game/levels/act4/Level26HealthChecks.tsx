...
export function Level26HealthChecks({ onComplete, onExit }: LevelComponentProps) {
  ...
  const getStatusColor = (status: HealthCheck['status']) => {
    switch (status) {
      case 'healthy': return 'text-success bg-success/10 border-success';
      case 'degraded': return 'text-warning bg-warning/10 border-warning';
      case 'unhealthy': return 'text-destructive bg-destructive/10 border-destructive';
        ...
        >
              ...
              {dependencies.map(dep => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span>{dep.icon}</span>
                    <span className={dep.healthy ? 'text-success' : 'text-destructive'}>
                      {dep.name}
                    </span>
                  </div>
          ...
          <div className="p-4 border-t border-border">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className={healthChecks.filter(c => c.enabled).length >= 2 ? 'text-success' : 'text-foreground'}>
          ...
          <div className="max-w-4xl mx-auto">
              ...
              {healthChecks.map(check => (
                <Button
                  ...
                  className={`h-auto rounded-xl border-2 overflow-hidden text-left transition-all flex-col p-0 ${
                    check.enabled ? getStatusColor(check.status) : 'border-border bg-card'
                  }`}
                >
                  <div className="p-4 w-full">
                    <div className="flex items-center justify-between mb-2">
                      <span className={`text-lg font-semibold ${check.enabled ? '' : 'text-muted-foreground'}`}>
                        {check.name}
                      </span>
                      {check.enabled && (
                        <span className={`w-3 h-3 rounded-full ${
                          check.status === 'healthy' ? 'bg-success' :
                          check.status === 'degraded' ? 'bg-warning animate-pulse' :
                          check.status === 'unhealthy' ? 'bg-destructive animate-pulse' : 'bg-muted-foreground'
                        }`} />
                      )}
                    </div>
                    <div className="text-xs text-muted-foreground mb-3">{check.description}</div>
                    <div className="space-y-1">
                      {check.checks.map((c, i) => (
                        <div key={i} className="text-xs text-muted-foreground flex items-center gap-1">
                          <span className={check.enabled && check.status === 'healthy' ? 'text-success' : ''}>
                  ...
                  <div className={`px-4 py-2 text-xs font-medium w-full ${
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
              <div className="bg-secondary px-4 py-3 border-b border-border">
                <div className="text-foreground font-semibold">Kubernetes Integration</div>
                <div className="text-xs text-muted-foreground">How K8s uses these endpoints</div>
              </div>
              <div className="p-4 grid grid-cols-3 gap-4">
                <div className="p-3 bg-secondary rounded-lg">
                  <div className="text-primary font-semibold text-sm mb-2">Liveness</div>
                ...
                <div className="p-3 bg-secondary rounded-lg">
                  <div className="text-primary font-semibold text-sm mb-2">Readiness</div>
                  <div className="text-xs text-muted-foreground">
                ...
                <div className="p-3 bg-secondary rounded-lg">
                  <div className="text-primary font-semibold text-sm mb-2">Deep</div>
                  <div className="text-xs text-muted-foreground">
                    GET /health/deep<br />
                    → For debugging<br />
                    → Not for K8s probes
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden">
              <div className="bg-secondary px-4 py-3 border-b border-border flex justify-between items-center">
                <div className="text-foreground font-semibold">Live Endpoint Status</div>
                {isMonitoring && (
              ...
              <div className="p-4 space-y-3">
                {healthChecks.filter(c => c.enabled).map(check => (
                  <div key={check.id} className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                    <div className="flex items-center gap-3">
                      <span className={`w-3 h-3 rounded-full ${
                        check.status === 'healthy' ? 'bg-success' :
                        check.status === 'degraded' ? 'bg-warning' :
                        check.status === 'unhealthy' ? 'bg-destructive' : 'bg-muted-foreground'
                      }`} />
                      <div>
                        <div className="text-foreground text-sm">{check.name}</div>
                        <div className="text-xs text-muted-foreground font-mono">/health/{check.id}</div>
                      </div>
                    </div>
                    <div className={`text-sm font-medium ${
                      check.status === 'healthy' ? 'text-success' :
                      check.status === 'degraded' ? 'text-warning' :
                      check.status === 'unhealthy' ? 'text-destructive' : 'text-muted-foreground'
                    }`}>
  ...
  def database_connected?
    ...
    false
        ...
        >
          <div className="p-4 border-t border-border">
            <div className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Routes</div>
...
get '/health/deep'`}
          ...
          <div className="p-4 border-t border-border">
            <div className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">K8s Config</div>
            <pre className="text-xs text-muted-foreground bg-secondary p-2 rounded overflow-x-auto">
{`livenessProbe:
  httpGet:
