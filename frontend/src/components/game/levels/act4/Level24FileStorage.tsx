...
const PROVIDERS = [
	...
	},
	{
		...
		pros: ['ML integration', 'Strong CDN'],
		cons: ['Less common', 'GCP-centric'],
	},
];

export function Level24FileStorage({
	onComplete,
	onExit,
}: LevelComponentProps) {
	...
	const simulateUpload = () => {
		...
		if (config.variants) {
			setTimeout(() => {
				setUploads((prev) =>
					prev.map((u) => (u.id === file.id ? { ...u, status: 'done' } : u)),
				);
			}, 2500);
		}
	...
	const selectProvider = (providerId: string) => {
		setConfig((prev) => ({
			...prev,
			provider: providerId as StorageConfig['provider'],
		}));
	};
	...
	const validateSolution = (): ValidationResult => {
		...
		return {
			valid: true,
			message: 'Production-ready file storage configured!',
		};
	};

	const handleComplete = async () => {
		const success = await completeLevel('act4-level24-file-storage', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
				...
				>
					...
					<div className="p-4 border-t border-border">
						...
						<div className="text-xs text-muted-foreground mt-1 text-center">
							{serverLoad > 70
								? 'High load! Enable direct uploads'
								: serverLoad > 40
									? 'Moderate load'
									: 'Healthy'}
						</div>
					...
					<div className="p-4 border-t border-border">
						<Button className="w-full py-2" onClick={simulateUpload}>
					...
					<div className="p-4 border-t border-border">
						<div className="flex justify-between text-sm mb-2">
							...
							<span
								className={
									config.provider && config.directUpload && config.cdn
										? 'text-success'
										: 'text-foreground'
								}
							>
								{
									[config.provider, config.directUpload, config.cdn].filter(
										Boolean,
									).length
								}{' '}
								/ 3 required
							...
							<div className="p-4 grid grid-cols-3 gap-4">
								{PROVIDERS.map((provider) => (
									...
									>
										...
										<div
											className={`font-semibold ${config.provider === provider.id ? 'text-success' : 'text-foreground'}`}
										>
											{provider.name}
										</div>
										...
										<div className="text-xs space-y-1">
											{provider.pros.map((p) => (
												<div className="text-success" key={p}>
													+ {p}
												</div>
											))}
											{provider.cons.map((c) => (
												<div className="text-destructive" key={c}>
													- {c}
												</div>
											))}
										</div>
								...
								{[
									{
										...
										name: 'Direct Upload',
										icon: '⬆️',
										desc: 'Browser uploads directly to S3, bypasses server',
									},
									{
										key: 'variants',
										name: 'Image Variants',
										icon: '🖼️',
										desc: 'Auto-generate thumbnails and sizes',
									},
									{
										key: 'cdn',
										name: 'CDN Delivery',
										icon: '🌍',
										desc: 'CloudFront/Cloudflare for fast global access',
									},
								].map((feature) => (
									<Button
										className={`p-4 h-auto rounded-lg border-2 text-left flex-col items-start transition-all ${
										...
										onClick={() =>
											toggleFeature(
												feature.key as 'directUpload' | 'variants' | 'cdn',
											)
										}
										variant="ghost"
									>
										...
										<div
											className={`font-semibold ${config[feature.key as keyof StorageConfig] ? 'text-success' : 'text-foreground'}`}
										>
											{feature.name}
										</div>
							...
							<div className="p-6">
								...
								<div className="flex items-center justify-between mb-6">
									...
									<div className="flex-1 relative h-20">
										{/* Server path */}
										<div
											className={`absolute top-0 left-0 right-0 flex items-center ${config.directUpload ? 'opacity-30' : ''}`}
										>
										...
										<div
											className={`absolute top-0 left-0 text-xs text-destructive ${config.directUpload ? 'opacity-30' : ''}`}
										>
											Through server (slow)
										</div>
										...
										{/* Direct path */}
										<div
											className={`absolute bottom-0 left-0 right-0 flex items-center ${!config.directUpload ? 'opacity-30' : ''}`}
										>
											<div
												className="flex-1 h-0.5 bg-success"
												style={{ marginTop: '2rem' }}
											/>
										</div>
										<div
											className={`absolute bottom-0 right-0 text-xs text-success ${!config.directUpload ? 'opacity-30' : ''}`}
										>
											Direct upload (fast)
										</div>
									</div>
