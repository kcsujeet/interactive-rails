        ...
        >
                ...
                >
                  <div className="flex items-center gap-2 w-full">
                    ...
                    <div className="flex-1">
                      <div className={feature.enabled ? 'text-success' : 'text-foreground'}>
                        {feature.name}
                      </div>
          ...
          <div className="p-4 border-t border-border">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className={features.filter(f => f.enabled).length >= 3 ? 'text-success' : 'text-foreground'}>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
              <div className="bg-secondary px-4 py-3 border-b border-border">
                <div className="text-foreground font-semibold">Architecture</div>
              </div>
              <div className="p-8">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-center">
                    <div className="w-48 p-4 bg-purple-600 rounded-xl">
                      ...
                      <div className="text-foreground font-semibold">API Gateway</div>
                      <div className="flex flex-wrap justify-center gap-1 mt-2">
                        {features.map(f => (
                          <span
                            key={f.id}
                            className={`text-xs px-2 py-1 rounded ${
                              f.enabled ? 'bg-success/30 text-success' : 'bg-secondary/50 text-muted-foreground'
                            }`}
                          >
                            {f.icon}
                          </span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {services.map(service => (
                      ...
                      >
                        ...
                        <div className={`text-xs ${service.healthy ? 'text-success' : 'text-destructive'}`}>
                          {service.name}
                        </div>
                      </Button>
                    ))}
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
              <div className="bg-secondary px-4 py-3 border-b border-border">
                <div className="text-foreground font-semibold">Simulate Requests</div>
              </div>
              <div className="p-4 grid grid-cols-5 gap-3">
                {services.map(service => (
                  ...
                  >
                    <div className="text-xl">{service.icon}</div>
                    <div className="text-xs text-primary font-mono">{service.path}</div>
                  </Button>
                ))}
                <Button
                ...
                >
                  ...
                  <div className="text-[10px] text-muted-foreground">Aggregated</div>
                </Button>
              </div>
            </div>
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden">
              <div className="bg-secondary px-4 py-3 border-b border-border">
                <div className="text-foreground font-semibold">Request Pipeline</div>
              </div>
                  ...
                  requests.map(req => (
                    <div key={req.id} className="p-3 bg-secondary rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-mono text-primary">{req.path}</span>
                        <span className={`text-xs px-2 py-1 rounded ${
                          req.status === 'completed' ? 'bg-success/40 text-success' :
                          req.status === 'rate-limited' ? 'bg-destructive/40 text-destructive' :
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {req.stages.map((stage, i) => (
                          <span key={i} className="text-xs text-muted-foreground bg-card px-2 py-1 rounded">
                            {stage}
                          </span>
                        ))}
                      </div>
...
server {
  ...
  location /api/orders {
    proxy_pass http://orders_service;
        ...
        >
          <div className="p-4 border-t border-border">
            <div className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Gateway Benefits</div>
