...
const INITIAL_DOMAINS: Domain[] = [
	...
	},
	{
		...
		name: 'Products',
		icon: '📦',
		description: 'Catalog, inventory, pricing',
		dependencies: [],
		extracted: false,
		color: '#22c55e',
	},
	{
		...
		name: 'Orders',
		icon: '🛒',
		description: 'Cart, checkout, order history',
	...
	},
	{
		...
		name: 'Payments',
		icon: '💳',
		description: 'Transactions, refunds, billing',
		dependencies: ['users'],
		extracted: false,
		color: '#ef4444',
	},
	...
	},
	{
		...
		name: 'Analytics',
		icon: '📊',
		description: 'Metrics, reports, dashboards',
...
export function Level35Microservices({
	onComplete,
	onExit,
}: LevelComponentProps) {
	...
	const extractDomain = (domainId: string) => {
		if (!canExtract(domainId)) {
			alert(
				`Extract dependencies first: ${domains
	...
	const returnToMonolith = (domainId: string) => {
		...
		const dependents = extractedDomains.filter(
			(d) => d.dependencies.includes(domainId) && d.extracted,
		);

		if (dependents.length > 0) {
			alert(
				`Cannot return: ${dependents.map((d) => d.name).join(', ')} depend on this service`,
			);
			return;
		}

		setDomains((prev) =>
			prev.map((d) => (d.id === domainId ? { ...d, extracted: false } : d)),
		);
	...
	const validateSolution = (): ValidationResult => {
		...
		return {
			valid: true,
			message: "Congratulations! You've mastered Rails architecture!",
		};
	};

	const handleComplete = async () => {
		const success = await completeLevel('act6-level35-microservices', {
			stars: 3,
		});
		if (success) {
			onComplete({ stars: 3 });
				...
				>
					...
					<div className="p-4 border-t border-border">
						<div className="flex justify-between text-sm mb-2">
							...
							<span
								className={
									extractedCount >= 3 ? 'text-success' : 'text-foreground'
								}
							>
						...
						<div className="grid grid-cols-2 gap-8">
							...
							<div className="bg-card rounded-xl border border-border overflow-hidden">
								<div className="bg-destructive/30 px-4 py-3 border-b border-border">
									...
									<div className="text-xs text-muted-foreground">
										{monolithDomains.length} domains remaining
									</div>
								</div>
								<div className="p-4">
										...
										<div className="space-y-3">
											{monolithDomains.map((domain) => {
												...
												const unmetDeps = domain.dependencies.filter(
													(depId) =>
														!domains.find((d) => d.id === depId)?.extracted,
												);
												...
												return (
													<div
														...
														key={domain.id}
														onClick={() =>
															extractable && extractDomain(domain.id)
														}
														style={{
															borderColor: extractable ? '#22c55e' : undefined,
														}}
													>
							...
							<div className="bg-card rounded-xl border border-border overflow-hidden">
								<div className="bg-success/30 px-4 py-3 border-b border-border">
									...
									<div className="text-xs text-muted-foreground">
										{extractedDomains.length} services deployed
									</div>
								</div>
								<div className="p-4">
									{extractedDomains.length === 0 ? (
										<div className="text-center py-8 text-muted-foreground">
											Click domains on the left to extract them as
											microservices.
											<br />
											<br />
											<span className="text-primary">Tip:</span> Start with
											services that have no dependencies!
										</div>
									) : (
										<div className="space-y-3">
							...
							<div className="mt-6 bg-linear-to-r from-primary/30 to-purple-900/30 border border-primary rounded-xl p-6">
								<div className="text-center">
									...
									<div className="text-muted-foreground max-w-lg mx-auto">
										You've completed the journey from "Hello Rails" to
										microservices architecture. You now understand how to build,
										scale, and maintain production Rails applications.
									</div>
								</div>
							</div>
						)}
