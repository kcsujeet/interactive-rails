...
function validateLevel11Solution(state: Level11State): ValidationResult {
  ...
  if (state.blockedCount < 3) {
    errors.push(`Need to block at least 3 unauthorized requests (currently ${state.blockedCount})`);
  }
...
describe('Level 11: Authorization', () => {
  describe('Initial State', () => {
    test('should be invalid when no policy added', () => {
      ...
      expect(result.details).toContain('Add the Policy node to protect your endpoints');
    });
    ...
    test('should require blocking at least 3 requests', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.details!.some(d => d.includes('block at least 3'))).toBe(true);
    });
  });
  ...
  describe('Security Breach', () => {
    test('should be invalid if breach occurred', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.details!.some(d => d.includes('security breach'))).toBe(true);
    });
