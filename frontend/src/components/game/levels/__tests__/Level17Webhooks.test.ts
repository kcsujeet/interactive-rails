...
function validateLevel17Solution(state: Level17State): ValidationResult {
  ...
  if (state.webhooksReceived < 2) {
    errors.push(`Receive at least 2 webhook callbacks (currently ${state.webhooksReceived})`);
  }
...
describe('Level 17: Webhooks', () => {
  describe('Initial State', () => {
    ...
    test('should require enabling webhook', () => {
      ...
      expect(result.details!.some(d => d.includes('webhook endpoint'))).toBe(true);
    });
    ...
    test('should require receiving webhooks', () => {
      ...
      expect(result.details!.some(d => d.includes('webhook callbacks'))).toBe(true);
    });
  });
