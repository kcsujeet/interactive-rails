...
function validateLevel23Solution(state: Level23State): ValidationResult {
  ...
  if (state.recsCircuitState !== 'open') {
    errors.push('Circuit should be open to protect from flaky Recommendations service');
  }

  if (state.feedFailures > 0) {
    errors.push(`Feed service should have zero failures (currently ${state.feedFailures})`);
  }

  if (state.feedSuccesses < 5) {
    errors.push(`Feed needs at least 5 successful requests (currently ${state.feedSuccesses})`);
  }
...
describe('Level 23: Circuit Breakers', () => {
  describe('Initial State', () => {
    ...
    test('should require enabling circuit breakers', () => {
      ...
      expect(result.details!.some(d => d.includes('circuit breakers'))).toBe(true);
    });
    ...
    test('should require zero feed failures', () => {
      ...
      expect(result.details!.some(d => d.includes('zero failures'))).toBe(true);
    });
