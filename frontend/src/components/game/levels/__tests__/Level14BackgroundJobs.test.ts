...
function validateLevel14Solution(state: Level14State): ValidationResult {
  ...
  if (state.completedJobs < 2) {
    errors.push(`Complete at least 2 background jobs (currently ${state.completedJobs})`);
  }

  if (state.workerEnabled && state.requestTime !== null && state.requestTime > 500) {
...
describe('Level 14: Background Jobs', () => {
  describe('Initial State', () => {
    ...
    test('should require experiencing blocking first', () => {
      ...
      expect(result.details!.some(d => d.includes('blocking behavior'))).toBe(true);
    });
