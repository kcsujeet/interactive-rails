...
function validateLevel20Solution(state: Level20State): ValidationResult {
	...
	if (state.rolloutPercentage < 100) {
		errors.push(
			`Increase rollout to 100% (currently ${state.rolloutPercentage}%)`,
		);
	}

	if (state.newSuccesses < 5) {
		errors.push(
			`Need at least 5 successful requests on new version (currently ${state.newSuccesses})`,
		);
	}
...
describe('Level 20: Feature Flags', () => {
	describe('Initial State', () => {
		...
		test('should require enabling flag', () => {
			...
			expect(result.details!.some((d) => d.includes('feature flag'))).toBe(
				true,
			);
		});
