...
function validateLevel18Solution(state: Level18State): ValidationResult {
	...
	if (state.directUploadsCompleted < 2) {
		errors.push(
			`Complete at least 2 direct uploads (currently ${state.directUploadsCompleted})`,
		);
	}

	if (state.memoryPeak >= 100) {
		errors.push(
			`Memory peak too high (${state.memoryPeak}MB) - direct upload should keep memory low`,
		);
	}
