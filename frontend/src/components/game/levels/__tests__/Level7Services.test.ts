...
function validateLevel7Solution(
	components: ServiceComponent[],
): ValidationResult {
	const errors: string[] = [];

	// Check all components are placed
	const unplacedComponents = components.filter(
		(c) => c.currentSection === null,
	);
	if (unplacedComponents.length > 0) {
		errors.push(
			`${unplacedComponents.length} component(s) still need to be placed`,
		);
	}
	...
	for (const component of components) {
		if (
			...
			component.currentSection !== component.correctSection
		) {
			const section = SECTIONS.find((s) => s.id === component.currentSection);
			errors.push(
				`"${component.name}" doesn't belong in ${section?.name || 'that section'}`,
			);
		}
	}
	...
	const actualStepOrder = stepsInSection
		.map((s) => s.stepOrder)
		.filter(Boolean);
	...
	if (actualStepOrder.length === 3) {
		for (let i = 0; i < actualStepOrder.length; i++) {
			if (actualStepOrder[i] !== expectedStepOrder[i]) {
				errors.push(
					'Steps must be in order: Validate → Charge Payment → Save & Notify',
				);
				break;
			}
		}
...
describe('Level 7: Building a Service Object', () => {
		...
		test('should be invalid when no components are placed', () => {
			...
			expect(
				result.details!.some((d) =>
					d.includes('8 component(s) still need to be placed'),
				),
			).toBe(true);
		});
	});
	...
	describe('Partial Solutions', () => {
		test('should be invalid when only dependencies are placed', () => {
			...
			expect(
				result.details!.some((d) =>
					d.includes('6 component(s) still need to be placed'),
				),
			).toBe(true);
		});
		...
		test('should be invalid when components are in wrong sections', () => {
			const components = SERVICE_COMPONENTS.map((c) => {
				...
				if (c.id === 'payment_gateway')
					return { ...c, currentSection: 'dependencies' };
			...
			expect(
				result.details!.some(
					(d) => d.includes('Constructor') && d.includes('Steps'),
				),
			).toBe(true);
		});
		...
		test('should be invalid when step placed in dependencies', () => {
			const components = SERVICE_COMPONENTS.map((c) => {
				...
				if (c.id === 'payment_gateway')
					return { ...c, currentSection: 'dependencies' };
				if (c.id === 'constructor')
					return { ...c, currentSection: 'initialize' };
				if (c.id === 'validate')
					return { ...c, currentSection: 'dependencies' }; // WRONG
				if (c.id === 'charge') return { ...c, currentSection: 'steps' };
				if (c.id === 'save_notify') return { ...c, currentSection: 'steps' };
				if (c.id === 'success') return { ...c, currentSection: 'results' };
				if (c.id === 'failure') return { ...c, currentSection: 'results' };
			...
			expect(
				result.details!.some(
					(d) => d.includes('Validate') && d.includes('Dependencies'),
				),
			).toBe(true);
		});
		...
		test('should be invalid when result placed in initialize', () => {
			...
			expect(
				result.details!.some(
					(d) => d.includes('Success Result') && d.includes('Initialize'),
				),
			).toBe(true);
		});
	});
	...
	describe('Component Classification', () => {
		...
		test('Payment Gateway should go to Dependencies', () => {
			const component = SERVICE_COMPONENTS.find(
				(c) => c.id === 'payment_gateway',
			)!;
			expect(component.correctSection).toBe('dependencies');
		});
	...
	describe('Section Requirements', () => {
		test('Dependencies section requires 2 components', () => {
			...
			const depsComponents = SERVICE_COMPONENTS.filter(
				(c) => c.correctSection === 'dependencies',
			);
		...
		test('Initialize section requires 1 component', () => {
			...
			const initComponents = SERVICE_COMPONENTS.filter(
				(c) => c.correctSection === 'initialize',
			);
		...
		test('Steps section requires 3 components', () => {
			...
			const stepsComponents = SERVICE_COMPONENTS.filter(
				(c) => c.correctSection === 'steps',
			);
		...
		test('Results section requires 2 components', () => {
			...
			const resultsComponents = SERVICE_COMPONENTS.filter(
				(c) => c.correctSection === 'results',
			);
	...
	describe('Service Object Pattern', () => {
		test('should have dependency injection via initialize', () => {
			const constructor = SERVICE_COMPONENTS.find(
				(c) => c.id === 'constructor',
			)!;
		...
		test('steps should follow validate-then-act pattern', () => {
			const steps = SERVICE_COMPONENTS.filter(
				(c) => c.correctSection === 'steps',
