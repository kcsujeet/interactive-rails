...
export function Level1StackChoice({ onComplete, onExit }: Level1StackChoiceProps) {
...
function getDatabaseInfo(db: DatabaseChoice) {
  if (db === 'postgresql') {
    return { name: 'PostgreSQL', description: 'Production-ready relational database', icon: '🐘', color: '#336791' };
  }
  if (db === 'sqlite') {
    return { name: 'SQLite', description: 'Simple file-based database', icon: '🪶', color: '#003b57' };
  }
  return null;
}
...
function getFrontendInfo(fe: FrontendChoice) {
  if (fe === 'hotwire') {
    return { name: 'Hotwire/ERB', description: 'Rails-native frontend with Turbo', icon: '⚡', color: '#ff6b6b' };
  }
  if (fe === 'react') {
    return { name: 'React', description: 'Modern SPA with API backend', icon: '⚛️', color: '#61dafb' };
  }
  return null;
}
