...
export function standardPipeline(opts?: PipelineOptions): PipelineState {
	...
	return {
		nodes: [
			...
			{ id: modelId, type: 'model', x: 660, y: 220, locked: true,
...
export function middlewarePipeline(opts?: PipelineOptions): PipelineState {
	...
	return {
		nodes: [
			...
			{ id: 'middleware-node', type: 'middleware', x: 220, y: 220, locked: true },
			{ id: 'router-node', type: 'router', x: 380, y: 220, locked: true },
			{ id: 'controller-node', type: 'controller', x: 540, y: 220, locked: true },
			{ id: modelId, type: 'model', x: 720, y: 220, locked: true,
				...(opts?.modelLabel ? { config: { label: opts.modelLabel } } : {}) },
			{ id: 'database-node', type: 'database', x: 900, y: 220, locked: true },
			{ id: 'serializer-node', type: 'serializer', x: 540, y: 400, locked: true },
