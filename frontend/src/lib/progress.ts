...
export async function completeLevel(options: {
  ...
  try {
    const response = await fetch(`/api/pipeline/levels/${options.levelId}/complete`, {
      method: 'POST',
...
export async function importGuestProgress(): Promise<{ success: boolean; message?: string }> {
  const guest = loadGuestProgress();
