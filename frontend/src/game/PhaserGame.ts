...
import type { Enemy, Defense, EnemyType, DefenseType } from '../stores/simulation';
...
const ENEMY_VISUALS: Record<EnemyType, { color: number; scale: number; shape: 'circle' | 'triangle' | 'diamond' }> = {
...
class GameScene extends Phaser.Scene {
  ...
  public fireProjectile(from: { x: number; y: number }, to: { x: number; y: number }, color: number) {
  ...
  private createDefenseSprite(defense: Defense): Phaser.GameObjects.Container {
    ...
    const container = this.add.container(defense.position.x, defense.position.y);
...
export class PhaserGameManager {
  ...
  fireProjectile(from: { x: number; y: number }, to: { x: number; y: number }, color: number = 0x22c55e) {
