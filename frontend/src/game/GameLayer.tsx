...
import { useSimulationStore, selectActiveEnemies, selectActiveDefenses } from '../stores';
  ...
  const handleDefenseAttack = useCallback(
    (defenseId: string, enemyId: string, damage: number) => {
      ...
      if (defense && enemy) {
        ...
        gameManagerRef.current.showDamage(enemy.position.x, enemy.position.y, damage);

        // Show kill effect if enemy died
        if (enemy.hp - damage <= 0) {
          setTimeout(() => {
            gameManagerRef.current?.showKill(enemy.position.x, enemy.position.y);
          }, 200);
        }
      }
    },
...
export function useGameLayer() {
  ...
  const fireProjectile = useCallback(
    (from: { x: number; y: number }, to: { x: number; y: number }, color?: number) => {
