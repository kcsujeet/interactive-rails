...
for (const act of ACTS) {
  for (const level of act.levels) {
    ...
    if (level.actId !== act.id) {
      errors.push(`Level ${level.id} actId (${level.actId}) does not match act ${act.id}`);
    }
    if (!level.levelNumber) errors.push(`Level ${level.id} missing levelNumber`);
    ...
    if (!level.trigger?.description) errors.push(`Level ${level.id} missing trigger description`);
    if (!level.startingPipeline) errors.push(`Level ${level.id} missing startingPipeline`);
    if (!level.problem?.observation) errors.push(`Level ${level.id} missing problem observation`);
    if (!level.learningContent?.title) errors.push(`Level ${level.id} missing learningContent title`);
    ...
    for (const node of level.availableNodes) {
      if (!nodeTypeSet.has(node)) {
        errors.push(`Level ${level.id} references unknown available node: ${node}`);
      }
    }
    for (const node of level.unlockedNodes) {
      if (!nodeTypeSet.has(node)) {
        errors.push(`Level ${level.id} references unknown unlocked node: ${node}`);
      }
    }
  }
}
...
if (allLevels.length !== expectedCount) {
  errors.push(`Expected ${expectedCount} total levels, found ${allLevels.length}`);
}
...
console.log(`Level validation passed: ${allLevels.length} levels across ${ACTS.length} acts.`);
