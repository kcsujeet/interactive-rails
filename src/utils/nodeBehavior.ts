...
export const NODE_BEHAVIORS: Record<string, NodeBehavior> = {
	...
	request: {
		...
		blockedConnections: [
			{
				target: 'database',
				reason: 'Requests cannot directly access the database',
			},
	...
	job_queue: {
		...
		blockedConnections: [
			{
				target: 'response',
				reason: 'Jobs are async and cannot directly affect the response',
			},
		],
	...
	mailer: {
		...
		blockedConnections: [
			{
				target: 'response',
				reason: 'Emails should be sent async to avoid blocking',
			},
		],
...
export const CONNECTION_COSTS: Record<
	string,
	Record<string, ConnectionCost>
> = {
	model: {
		database: {
			latency: 50,
...
export function isConnectionAllowed(
	...
	const blocked = sourceBehavior.blockedConnections.find(
		(b) => b.target === targetType,
	);
	if (blocked) {
...
export function detectNPlusOnePattern(
	...
	for (const model of modelNodes) {
		...
		const dbConnection = connections.find(
			(c) =>
				c.sourceId === model.id &&
				nodes.find((n) => n.id === c.targetId)?.type === 'database',
		);
		...
		if (dbConnection) {
			...
			const hasEagerLoad = eagerLoadNodes.some((el) => {
				...
				const elToDb = connections.find(
					(c) => c.sourceId === el.id && c.targetId === dbConnection.targetId,
				);
				const modelToEl = connections.find(
					(c) => c.sourceId === model.id && c.targetId === el.id,
				);
...
/**
 * BFS to find the path from request to response through the pipeline
 */
...
/**
 * Determines query particle visualization settings based on level type.
 * Returns null for non-performance levels (no particles needed).
 */
