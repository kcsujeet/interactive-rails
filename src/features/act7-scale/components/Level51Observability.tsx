...
const LogNode = memo(function LogNode({ data }: { data: LogNodeData }) {
	...
	return (
		...
		>
			<FlowHandles />
			<FileText className={`w-5 h-5 mx-auto mb-1 ${iconClass}`} />
...
const TraceNode = memo(function TraceNode({ data }: { data: TraceNodeData }) {
	...
	return (
		...
		>
			<FlowHandles />
			<Activity className={`w-5 h-5 mx-auto mb-1 ${iconClass}`} />
			<div className="text-sm font-semibold text-foreground">{data.label}</div>
			{data.sublabel && (
				<div className="text-xs text-muted-foreground mt-0.5">
					{data.sublabel}
...
const HealthNode = memo(function HealthNode({
	...
	return (
		...
		>
			<FlowHandles />
			<Heart className={`w-5 h-5 mx-auto mb-1 ${iconClass}`} />
			<div className="text-sm font-semibold text-foreground">{data.label}</div>
			{data.sublabel && (
				<div className="text-xs text-muted-foreground mt-0.5">
					{data.sublabel}
...
export function Level51Observability({ onComplete }: LevelComponentProps) {
