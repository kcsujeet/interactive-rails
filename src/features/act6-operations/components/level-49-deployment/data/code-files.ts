...
gem "rails", "~> 8.0"
gem "kamal", "~> 2.0"  # added by rails new
gem "puma"`,
		});
	}
	...
	if (completedStep >= 2) {
		files.push({
			...
			language: 'yaml',
			code: `# Generated by rails new. Fill in the essentials.
service: my_app
image:
servers:
registry:
...
DATABASE_URL=$(op read "op://prod/app/DATABASE_URL")`,
	...
	if (files.length === 0) {
		files.push({
			...
			language: 'bash',
			code: `# rails new already scaffolded Kamal (Gemfile, Dockerfile,
# config/deploy.yml, .kamal/secrets). Nothing is configured
# or deployed yet. Confirm the tool, then customize the config.`,
		});
	}
