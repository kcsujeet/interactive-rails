...
**Damage-first check (NON-NEGOTIABLE — CRITICAL severity if it fails).** See `pedagogy.md` § "Show the damage, then introduce the fix". The visualization's headline visual weight must rest on the customer-facing damage that occurs when this level's concept is missing — not on artifacts (a spec file, a config file, a data-flow zone lane), not on tools (an Editor node, a Test Runner node), not on abstract status grids (`?` / `✓` / `✗` cards), not on Rails-internal pipeline metaphors (Incoming Request → Normalizes → Model → After Save). For each level under audit, list the visualization's primary visual elements and ask: *"is this the damage, or is it just the artifact / tool / status / dev-pipeline?"* If anything except the damage carries the headline visual weight, **FAIL with CRITICAL severity** — the level cannot ship until the visualization is redesigned, regardless of how many other checks pass. Demoting this finding to "minor" because other drifts feel more fixable is the audit-discipline trap re-running.

**Smell test (mandatory output).** Per `pedagogy.md` § "The smell test", before claiming any audit pass, answer in writing: *"If a first-time developer with no Rails background sees this observe phase, would their reaction be 'this is bad, I have to fix this' — or 'I don't understand what I'm looking at'?"* The answer goes in the audit output. If it isn't the first option, the audit verdict is FAIL.

**Lens-on-touch rule.** When auditing a level that was designed before the consequences-first lens was codified (2026-05-09), the visualization itself is in scope — not just the data / content drift. Pre-lens visualizations are *presumed drift* until they pass the damage-first check and the smell test. Treating a pre-lens visualization as canonical and working around it is the trap.

**Worked-failure case studies.** L14 (Testing): three iterations polished the wrong answer (pipeline / tool nodes / status cards) before landing on a customer-impact dashboard. L15 (Callbacks): touched the day after the lens was codified; existing zone-lane visualization (Incoming Request → Normalizes → Product Model → After Save) was kept and labels rephrased — it shipped failing the smell test ("Normalizes (Product) (skipped)" is meaningless to a first-time dev) because the audit downgraded `show-the-damage` to "minor". Both case studies live in `pedagogy.md` § "Show the damage, then introduce the fix".
...
- **Phase 2 (problem solving)** — gem install + generator + migration steps included, code preview honesty, option quality (3 options shuffled, same color, no answer leaks in feedback), the dedicated **step scope scan** (on-concept + non-redundant), the dedicated **feedback answer leak scan**, AND the dedicated **observe code preview answer-leak scan**. All three are separate mandatory passes — easy to skip when burying inside option quality / phase-1 review. The step scope scan asks "do N steps teach N lessons, or fewer with redundancy and off-concept padding?" — write each step's one-sentence lesson and inspect the list as a whole, not just step-by-step. The observe scan covers `getCodeFiles('observe', ...)` for two distinct shapes (reference-file leak, comment-text leak). All three shapes worked-cased on L15 (2026-05-09).
